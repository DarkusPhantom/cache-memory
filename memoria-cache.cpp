...
void direct_cache_process(list<int> direction_list, int capacity_cache) {
    ...
    cout << "Generando la correspondencia directa..." << endl;
...
int main() {
    ...
    if (emplazamiento == 0){
    	...
    	if (replace_politic == 0) {
    		direct_cache_process(direction_list, number_of_lines);
    	} else {
    		//replaceFIFO()
    	}
    } else {
